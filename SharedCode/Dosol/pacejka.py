...
# closed_truck.stl 파일, 3톤 4륜 후륜구동 대형트럭 기준 조사

# ► 차량 제원
WHEEL_BASE      = 7.30   # m   휠베이스
TRACK_HALF      = 1.90   # m   트랙 반차폭
CG_HEIGHT       = 0.90   # m   무게중심 높이 (model에서 읽으면 치환됨)   # 차체 높이 0.9m 가정
WHEEL_RADIUS    = PARAMS["WHEEL_DIAMETER"]   # m
JOINT_DAMPING   = 0.10   # N·m·s/rad  (휠 축 마찰)                    # 0.1 가정

# ► Pacejka 종/횡 계수 (Bx,Cx,Ex,μ)                         마른 아스팔트          젖은 아스팔트        눈 덮인 아스팔트
BX, CX, EX, MU_X = 11.4, 1.9, 0.97, 0.9   # Longitudinal   11.4 1.9 0.97 0.9    12.6 2.3 1.0 0.7   4.7 2.0 1.0 0.24
BY, CY, EY, MU_Y =  8.1, 1.3, 0.97, 1.0   # Lateral        8.1 1.3 0.97 1.0     9.0 1.5 1.0 0.78   3.8 1.8 1.0 0.27
MAX_LOAD         = 15000.0  # N  휠당 최대 하중(클램프)       수화물 포함 최대 6톤 가정

# ► 슬립 한계
SLIP_LIMIT       = 0.20   # |κ| ≤ 0.2 (±20%)                                                       0.15
